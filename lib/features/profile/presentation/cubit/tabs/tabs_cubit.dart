...
class TabsCubit extends Cubit<TabsState> {
  ...
  Future<void> checkIfBookmarked(int movieId) async {
    try {
      ...
      emit(WatchListUpdated(doc.exists));
    } catch (e) {
      emit(TabsError('Something went wrong'));
    }
  }
    ...
    try {
      ...
      if (!doc.exists) {
        ...
        await _firestore
            ...
            .doc(_auth.currentUser!.uid)
            .update({
          'watchListCount': FieldValue.increment(1),
        });
      }

      emit(WatchListUpdated(true));
    } catch (e) {
      emit(TabsError('Something went wrong'));
    }
  }
